...
function ProjectCard({
  ...
  const slug = title ? title.toLowerCase().replace(/\s+/g, "-") : "project"
  return (
    <a href={`/project/${slug}`} className="block focus:outline-none" style={{ textDecoration: "none" }}>
      <div
        ...
        style={{
          boxShadow: `inset -6px -6px 10px 0 ${innerShadowColor || "rgba(255,255,0,0.25)"}, 2px 2px 6px #23272f, -2px -2px 6px #3a3f4b`,
        }}
      >
        <div className="text-white font-bold text-lg">{title}</div>
        <div
          className="text-[#BCB4B4] text-sm sm:text-base mb-2 font-light flex-1"
          style={{ fontFamily: "HelveticaNeueThin, Helvetica Neue, Arial, sans-serif" }}
        >
          {description}
        </div>
        <div className="flex items-center gap-2 text-[#A6CFFE] text-sm">
        ...
        <div
          className={`w-full bg-[#1a2233] rounded-full h-3 mb-2 p-0.5 border ${safeProgress === 100 ? "border-green-500" : "border-blue-500"} box-border`}
        >
          <div
            className={`h-full rounded-full ${safeProgress === 100 ? "bg-green-500" : "bg-blue-500"}`}
            style={{ width: `${barWidth}%` }}
        ...
        <div className="w-full flex justify-end -mt-3 mb-1">
          <span className={`text-xs ${safeProgress === 100 ? "text-green-500" : "text-blue-300"} font-semibold`}>
